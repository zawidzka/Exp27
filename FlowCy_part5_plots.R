...
# BM vs LN
plotDiffHeatmap(sce, da1, top_n = 8, all = TRUE, fdr = FDR_cutoff)
stat_test1 <- as_tibble(da1)
p.adj.signif_1 <- c("ns", "ns", "*", "*", rep("ns", 3), "*")
group1 <- (rep("BM", nrow(stat_test1)))
group2 <- (rep("LN", nrow(stat_test1)))
y.position <- c(60, 60, 80, 30, 30, 10, 40, 60)
...
plotDiffHeatmap(sce, da2, top_n = 8, all = TRUE, fdr = FDR_cutoff)
stat_test2 <- as_tibble(da2)
p.adj.signif_2 <- c(rep("*", 3), "ns", "*", "ns", "*", "ns")
...
# LN v TUM
plotDiffHeatmap(sce, da3, top_n = 8, all = TRUE, fdr = FDR_cutoff)
stat_test3 <- as_tibble(da3)
p.adj.signif_3 <- c(rep("*", 5), "ns", "*", "*")
...
plotClusterHeatmap(sce,
                   hm2 = NULL, k = "meta8", m = NULL, cluster_anno = TRUE, draw_freqs = TRUE)
plotClusterExprs(sce, k = "meta8", features = "state")
